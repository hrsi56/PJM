# Engineering Plan — PJM Western Hub Day-Ahead Price Forecasting Tool, v5.5 — Lean DS Capstone with Cloud-Backed Showcase
...
1. **Out-of-sample length vs. compute — RATIFIED: per-fold recalibration + ~450 test-days, mixed across regimes** (Section 5.1). Narrowing the training window to extend the test span is **rejected** — it would cost the pre-/post-Elliott regime span that the whole methodology depends on. The ~450-day, per-fold, mixed-regime design is the deliberate $0-compute balance. *(v5.5 note: the eval months are re-pinned — Fold 4 becomes a Jan–Mar 2025 deep-winter block, Fold 5 slides to end at the most-recent data available at the M1 pull. Per-fold recalibration and the ~450 mixed-regime test-days are unchanged.)*
...
3. **Western Hub temperature zone — RATIFIED: load-weighted aggregate over the Western-Hub-adjacent zones in the catalog (AEP, APS, Dominion, PEPCO, PPL, MetEd)** (Section 4.1). A single station is **rejected** — it injects micro-climate noise irrelevant to the hub. *(v5.5: the load weights are defined as train-only statistics, refit per fold — Section 4.1.)*
...
**v5.4 → v5.5 delta.** Eleven changes from the change-anchoring register, adjudicated in the orchestrator decision register (2026-06-10). None adds modeling scope; no §13 boundary is reopened except the testing entry. Grouped:

*Validation & gates:* (1) **Fold 4 re-pinned to a deep-winter eval block (Jan–Mar 2025)** so `cold_weather_alert` days and the post-Elliott premium — the headline narrative — are tested truly out-of-sample (§5.1; §0 item 1 note); (2) **training window now runs 2019-01-01 → the actual M1 pull date**, and Fold 5 slides to end at the most-recent available data, keeping the headline eval fresh for late-2026 interviews (§3, §5.1); (3) **CP-2 gains a pre-committed secondary pass condition** — if Fold-5 DM p ∈ [0.05, 0.15] but the pooled-across-folds daily-vector DM is significant, M2 passes and both numbers are reported (§7.1, CP-2); (4) **CQR calibration set pinned** to the final 60–90 days of each fold's training partition (§6.2).

*Leakage & data risk:* (5) **new risk R-4** — `cold_weather_alert` historical issuance-time sourcing, with a pre-authorized conservative date-level fallback (§4 regime indicators, §5.2, §11); (6) **load-weighted temperature-aggregate weights defined as train-only statistics**, refit per fold — previously a latent leakage vector (§4.1, §0 item 3).

*Precision & honesty:* (7) **CQR no-crossing claim corrected** for the negative-threshold case (§6.2); (8) **model staleness disclosed** — frozen training cutoff vs. current snapshot data — in the reproducibility statement, limitations, and README (§9.3, §10); (9) **M1 redistribution-terms check** on the committed PJM snapshot, with an engineered-matrix + regeneration-code fallback (§9.3, CP-1); (10) **`predict_next_week.py` renamed `predict_next_day.py`** — the product is a D+1 forecast (§9.2).

*Engineering signal:* (11) **thin CI** — a GitHub Actions workflow runs the §9.4 invariant tests on every push; still no regression matrix (§9.4, §13).

*Reviewed and rejected (boundaries stand):* a documented neural baseline (SC3) and an EnbPI conformal comparison (SC5) were considered and **rejected** — the §13 "no neural challenger" and "no sequential conformal" entries are unchanged, except that the sequential-conformal entry now carries a single pre-committed reopen condition (post-Elliott empirical coverage diverging from nominal by >10 percentage points at M3 → EnbPI returns as remediation, comparison only).
...
**Implication for the model.** The price distribution is not stationary. The system therefore mandates walk-forward CV that spans both regimes, explicit regime indicator features, and per-regime calibration checks — all covered below. The 2019→pull-date training window (Section 3) spans both regimes while keeping the pre-Elliott era from dominating the data the forecast actually serves.

## 3. Data Sources

All data is free. The training window is **2019-01-01 → the actual M1 data-pull date (~6.5+ years; endpoint finalized at the M1 pull)** — long enough for ample data and a mixed-regime fold scheme, recent enough to keep the pre-Elliott era from dominating, fresh enough that the headline eval fold ends at current data, and small enough on a 16 GB machine. (Hub-level DA LMP is available back to ~2010–2011 via the archive endpoints; the window is a deliberate scope choice, not a data limit.)
...
**Regime indicators (KFT — known a day ahead)** — `post_elliott` binary; `cold_weather_alert` flag, using only the alert state **known at the D0 11:00 gate** (confirm the PJM Cold Weather Advisory/Alert for the target day is issued before the gate; if issued after, it is look-ahead — Section 5.2). **Historical sourcing is risk R-4 (Section 11):** 2019→present alert records *with issuance timestamps* are not a Data Miner 2 feed. **Pre-authorized degraded fallback:** if issuance times cannot be reconstructed, use date-level alert flags under the conservative rule that an alert is treated as known only from the day **after** issuance — no look-ahead by construction, at the cost of a one-day lag on alert onset — and document the choice in the §5.2 leakage audit.
...
| 4 | 2025-01-01 → 2025-03-31 | post-Elliott; **deep-winter stress block** — the cold-snap months where `cold_weather_alert` days and the post-Elliott premium live |
| 5 | final 90 days ending at the most-recent available data at the M1 pull (dates pinned at M1) | post-Elliott; current-data tail |
...
**Recalibration cadence.** At each fold boundary, re-fit the LightGBM heads, the CQR thresholds, **and** the climatology; within a 90-day block, predict D+1 sequentially without peeking. Per-day recalibration (epftoolbox gold standard) is out of scope under the $0 compute ceiling; **per-fold recalibration is the stated lean substitute** (Owner item 1). Fold 4 (Jan–Mar 2025) supplies the **true out-of-sample winter-stress block** — real `cold_weather_alert` days for the regime-stratified table (Section 8.3) rather than backtest-only rows; Fold 5 ends at the most-recent data, keeping the headline eval fresh; Fold 2 adds spring to balance the summer-heavy Folds 1/3, and Fold 5's season falls where the pull date lands.
...
- **Three audited vectors:** (i) rolling-quantile features computed closed-left so the target is never in its own window; (ii) `cold_weather_alert` issuance confirmed before the D0 gate (otherwise look-ahead) — if issuance timestamps are unrecoverable (R-4), the pre-authorized degraded rule applies: date-level flags, alert treated as known only from the day **after** issuance; (iii) `post_elliott` leaks nothing as a constant but is perfectly collinear with time inside any all-post-Elliott fold — a further reason for the mixed-regime scheme (Section 5.1).
...
**Snapshot redistribution check (v5.5, verified at M1):** confirm that PJM Data Miner 2 terms permit **public redistribution** of the committed data snapshot. If redistribution is restricted, the fallback is to commit the **engineered feature matrix plus the ingestion code that regenerates the raw pulls from the free API** — reproducibility is preserved and the redistribution question is mooted. EIA and NOAA data are US public domain — no issue. (Cross-ref R-3; CP-1 carries the checklist item.)

**Model-staleness disclosure (v5.5, lands at M5):** the reproducibility statement, the honest-limitations section (Section 10, item 11), and the README each state the **training-data cutoff** (the M1 pull date) and the fact that the deployed demo applies a **frozen model** to later weekly data snapshots it never trained on. One sentence each; honesty, not apology.
...
**Thin CI (v5.5, lands at M2 once the invariant tests exist):** a minimal GitHub Actions workflow runs these invariant tests **on every push** — a visible green check on the public repo, at $0 (Actions free tier) and well under an hour to wire. This is deliberately **not** a regression matrix: no test pyramid, no scheduled retraining, no drift gates, no rollback machinery (Section 13 boundary, reworded accordingly). It converts "I wrote invariant tests" into "the invariants are verified on every commit."
...
### R-4 — `cold_weather_alert` historical issuance-time sourcing (v5.5)
**Description.** The `cold_weather_alert` feature needs 2019→present PJM Cold Weather Advisory/Alert records **with issuance timestamps** to certify the KFT claim (alert state known at the D0 11:00 gate). This is **not** a Data Miner 2 feed — the records live in PJM emergency-procedures postings and may not be cleanly timestamped or machine-readable across the full window. **Mitigations:** availability is probed in the **Month-0 data-layer de-risking spike**, months ahead of M1, so a gap surfaces as a planning fact rather than an M1 stall; the **pre-authorized degraded fallback** (Section 4) — date-level alert flags under the conservative rule that an alert is treated as known only from the day **after** issuance (no look-ahead by construction; one-day lag on onset) — is applied at M1 if timestamps cannot be reconstructed, and documented in the §5.2 leakage audit. The conservative direction is deliberate: it can only *understate* the feature's usefulness, never leak.

## 12. Milestones and Checkpoints

**Build-routing note (orchestrator-facing).** No milestone is briefed inside this document. Each is briefed by the **orchestrator** at the session its work begins, as a Type B-Claude block to the engineering lead, **pointing the engineer to this document** for architecture and acceptance criteria — never restating them. Briefs are one-off and never pre-written here. **The next brief due is the M1 brief** (data layer + feature catalog v1), generated when M1 is scheduled (syllabus Month 2, per `syllabus_v2_2.md`). It must carry the v5.5 decisions binding M1: **24h D+1 target** (§1); **UTC indexing + DST handling** (§4.0); **verified DA LMP target feed** and **bid-gate load-forecast vintage** (§3); **PJM-native weather + the 2019-01-01 → pull-date window, with the window endpoint and Fold 5's dates pinned at the pull** (§3, §5.1); the **train-only Fourier climatology** headline feature with `scarcity_stress` secondary and **train-only temperature-aggregate load weights** (§4.1); the **pinned mixed-regime fold scheme with the Jan–Mar 2025 winter Fold 4** (§5.1); the **ratified §0 decisions** (per-fold/450-day, gas fallback, load-weighted temperature zone); the **R-4 alert-sourcing check and its pre-authorized fallback** (§4, §11); the **PJM redistribution-terms check** (§9.3); and the **invariant tests** (§9.4). Until that session, no M1 brief exists.

### M0 — Plan approved
This document (v5.5).

### M1 — Data layer
Bulk pull from PJM Data Miner 2 (verified DA LMP + decomposition, bid-gate-vintage load forecast, `da_temperature_sets`) + EIA gas, 2019-01-01 → the pull date (window endpoint and Fold 5 dates pinned here), UTC-indexed. NOAA NDFD only if its forecast archive proves clean (R-2). Feature catalog v1 including both domain-insight candidates, the train-only climatology, the train-only temperature-aggregate weights, and the regime indicators (with the R-4 alert-sourcing outcome applied). KFT/LAG classification documented with the weather-source guard and the three leakage-vector audits.
...
- [ ] Historical bulk pull covers **2019-01-01 → the pull date**, UTC-indexed, DST handled (23/25-hour days correct), no nulls in `total_lmp_da`, SMP, MCC, MLC for Western Hub (`pnode_id 51217`), from the **verified** feed. Window endpoint and **Fold 5's eval dates pinned** from the pull date (§5.1).
...
- [ ] `post_elliott` flips at 2023-01-01; `cold_weather_alert` decodes from gate-known alert state with no false positives in a spot-check — **or**, if issuance timestamps proved unrecoverable, the **R-4 degraded fallback** (date-level flags, alert known only from the day after issuance) is applied and documented in the §5.2 audit.
- [ ] Weather features are forecasts (KFT), not realized actuals (R-2 smoke test).
- [ ] Train-only climatology fit (per-fold), coefficients persisted in the pyfunc; `temperature_deviation_from_normal` computable at inference with no external call. Temperature-aggregate **load weights computed on training rows only, refit per fold** (§4.1).
...
- [ ] **PJM Data Miner 2 redistribution terms checked** for the committed snapshot; if restricted, the engineered-matrix + regeneration-code fallback is adopted and documented (§9.3).
...
- [ ] **Thin CI live (v5.5):** a GitHub Actions workflow runs the §9.4 invariant tests on every push; green on `main`. No regression matrix.
...
- [ ] The honest limitations section names the exchangeability violation, the regime-stratified gaps, the bounded-target tail caveat, any coverage divergence, and the **model-staleness disclosure** (frozen training cutoff vs. current snapshot data — also stated in the README).
- [ ] Reproducibility statement present and accurate (seeds, pinned versions, snapshot at tagged release, DagsHub permalinks, compute footprint, **training-data cutoff stated**).
...
Each omission is a deliberate scope decision, defended and interview-quotable. None was reopened by v5.3. The v5.5 review explicitly considered and **rejected** reopening two of them — a documented neural baseline and an EnbPI comparison — so the boundaries below were re-ratified, not merely inherited; the only edits are the thin-CI testing entry and the single pre-committed EnbPI reopen condition.
...
**No sequential conformal (EnbPI / SPCI) and no Giacomini-White.** *"CQR gives finite-sample marginal coverage under exchangeability — the property an interview reader is most likely to recognize and probe. EnbPI/SPCI handle non-exchangeability more aggressively but add conceptual surface that distracts from the core argument. The reliability diagram makes the exchangeability violation visible, and the limitations section names it."* **Pre-committed reopen condition (v5.5, the only one):** if the M3 reliability diagram shows post-Elliott empirical coverage diverging from nominal by **more than 10 percentage points**, EnbPI returns as **remediation** — a documented comparison only; CQR remains the shipped calibration inside the champion `pyfunc`. Below that threshold the boundary stands as written.
...
**No test matrix, no regression CI — but targeted invariant tests on a thin CI.** *"I wrote a few focused property tests on the invariants that silently corrupt results if violated — quantile monotonicity after the CQR-plus-isotonic pipeline, closed-left rolling windows, and UTC/DST lag alignment — and a thin GitHub Actions workflow runs them on every push. That's the right testing level for a single-developer portfolio: assert the properties that must hold and verify them automatically on every commit, not a regression matrix on a script that runs in five minutes, which would be engineering theater. Still no scheduled retraining, no drift gates, no rollback machinery. The invariant tests plus the thin CI show software-engineering judgment; a sprawling CI matrix would show the opposite."*

---

*End of Engineering Plan — PJM Western Hub Day-Ahead Price Forecasting Tool, v5.5 — Lean DS Capstone with Cloud-Backed Showcase.*
