...
1. **Phase-trigger (continuous activation).** **After capstone M2 lands** (Ridge + seasonal-naïve baselines + LightGBM quantile ensemble + Diebold-Mariano test all shipped, MLflow showing ≥5 experiments). Projected mid-Month 3 of the v2.2 syllabus. From M2 forward, Track C becomes continuously active — applying, recruiter outreach, interview prep, target-company research. Active applications begin no later than the start of syllabus Month 5. This is the application phase; rules 1, 2, and 4 no longer apply, Track C is first-class.
...
- **[APPLIED-AUTH] — use library, understand output, recognition-level theory:** the change-point block is the canonical example. Yarden runs the tool, understands the output, and can defend the result in interview — but doesn’t author the algorithm from scratch.
